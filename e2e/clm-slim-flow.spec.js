...
test('KPI 카드 4개 표시 + 만료 임박 카드 클릭 시 필터 적용', async ({ page }) => {
  ...
  await page.waitForSelector('#ct-kpi-wrap .ct-kpi-card', { timeout: 15000 });

  const cards = page.locator('.ct-kpi-card');
  ...
  // 검토중 / 진행중 / 초안
  await expect(page.locator('.ct-kpi-card', { hasText: '검토중' })).toContainText('7');
  await expect(page.locator('.ct-kpi-card', { hasText: '진행중' })).toContainText('20');
  await expect(page.locator('.ct-kpi-card', { hasText: '초안' })).toContainText('5');
...
test('[+ 새 계약] 클릭 → 모드 chooser 모달 (계약서 받음 vs 빈 양식부터)', async ({ page }) => {
  ...
  // 안내 메시지
  await expect(page.locator('text=둘 중 어떤 모드로 시작하든')).toBeVisible();
});
...
test('파일 첨부 모드 → 임시 인트로 + AI 카드 안에 [📎 파일 첨부] 버튼 노출', async ({ page }) => {
  ...
  // 임시 인트로 + AI 카드 노출 확인
  await page.waitForSelector('text=파일 첨부 → AI 법무 분석 → 자동 채움', { timeout: 10000 });
  await expect(page.locator('text=임시 계약번호')).toBeVisible();
