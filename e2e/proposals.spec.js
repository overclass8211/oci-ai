...
test('Phase 6-A — 신규 등록 모달: 4개 탭 표시 + 기본 외 탭 disabled', async ({ page }) => {
  await page.goto('/#proposals');
  await page.waitForSelector('#pr-new-btn', { timeout: 15000 });
  ...
  // 4개 탭 모두 표시 (기본+RFP / AI / 자료+견적 / 발송+이력)
  await expect(page.locator('.pr-tab')).toHaveCount(4);

  // 기본 탭은 활성
  await expect(page.locator('.pr-tab.active')).toContainText('기본');

  // 신규 모드 — 나머지 3개 탭은 disabled
  const disabledTabs = ['ai', 'content', 'send'];
...
test('Phase 2 — 편집 모달: 모든 탭 활성 + 탭 전환 동작', async ({ page }) => {
  ...
  await page.locator('.pr-tab[data-tab="send"]').click();
...
test('Phase 4-C — RFP/자료 탭 드롭존 + AI 분석 버튼 표시', async ({ page }) => {
  ...
  // Phase 6-A: 기본 탭에 RFP 섹션 통합 — 모달 열자마자 RFP 드롭존 표시
...
test('Phase 4-D — AI 탭: RFP 파일 없으면 분석 버튼 비활성 + 빈 상태 안내', async ({ page }) => {
  ...
  await page.locator('.pr-tab[data-tab="ai"]').click();
  // RFP 파일 없음 안내 (Phase 6-A: 기본 탭에 통합됨)
  await expect(page.locator('#pr-tab-content')).toContainText('기본 탭의 RFP 영역에서');
  // 분석 버튼 비활성
...
test('Phase 6-C — AI 평가 mock 호출 → 결과 카드 렌더 (커버율 / 충족 / 누락 / 개선)', async ({
  ...
  // 자료 & 견적 탭 → [📊] 클릭
  await page.locator('.pr-tab[data-tab="content"]').click();
  await page.locator('.pr-file-evaluate[data-id="41"]').click();
