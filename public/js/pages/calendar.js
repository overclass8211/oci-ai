...
const CalendarPage = (() => {
    ...
    document.getElementById('cal-save-btn').addEventListener('click', async () => {
      ...
      try {
        ...
        if (syncAct && data.lead_id && calId && data.start_datetime) {
          const evtToAct = { '미팅':'meeting', '영업방문':'site_visit', '제안':'proposal', '입찰':'bidding', '내부':'note', '기타':'note' };
          try {
        ...
        Toast.success(syncAct && data.lead_id ? '일정 등록 + 활동 이력 기록 완료' : '일정이 등록되었습니다');
        Modal.close();
        calendar?.refetchEvents();
      } catch (_) { /* API error handled by Toast elsewhere */ }
    });
  }
  ...
  function openDetailModal(ep) {
    ...
    const descBlock = ep.description ? (() => {
      ...
      const cleanDesc = ep.description.replace(/\n?\[회의록 상세보기\]\s*meeting:\d+/, '').trim();
    ...
    const ACTIVITY_ICON_MAP = {
      meeting:'🤝', call:'📞', email:'✉', site_visit:'🏗',
      proposal:'📄', bidding:'📋', contract:'✍', note:'📝'
    };
    const linkedActivityBlock = ep.linked_activity_id ? (() => {
      ...
      const performer = ep.linked_activity_performer ? ` · ${esc(ep.linked_activity_performer)}` : '';
      const leadClickAttr = ep.lead_id ? `id="cal-go-lead-btn" data-lead-id="${ep.lead_id}" style="cursor:pointer"` : '';
      return `
        <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border)">
  ...
  function openEventById(eventId, dateStr) {
    ...
    // 날짜로 이동
    if (dateStr) {
      try { calendar.gotoDate(dateStr); } catch (_) {}
    }
    ...
    setTimeout(() => {
      ...
      if (fcEvent) {
        ...
        API.get(`/calendar/events/${eventId}`)
          .then(r => { if (r.success && r.data) openDetailModal(r.data); })
