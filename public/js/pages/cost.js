...
const CostPage = {
  ...
  async render() {
    document.getElementById('content').innerHTML = `
      <div class="tab-bar">
        <button class="tab-btn ${this.activeTab==='product'?'active':''}" onclick="CostPage.switchTab('product')">상품 원가</button>
        <button class="tab-btn ${this.activeTab==='calculator'?'active':''}" onclick="CostPage.switchTab('calculator')">프로젝트 원가 산정</button>
        <button class="tab-btn ${this.activeTab==='history'?'active':''}" onclick="CostPage.switchTab('history')">원가 변동 이력</button>
      </div>
    ...
    const html = `
      <div class="flex-between mb-3">
        ...
        <div class="flex gap-2">
          <button class="btn btn-ghost" onclick="CostPage.syncOnERP()">🔄 OnERP 동기화</button>
          <button class="btn btn-primary" onclick="CostPage.openProductForm()">+ 항목 추가</button>
        </div>
      </div>
            ...
            <tbody>
              ${this.products.map(p => `
                <tr>
                  ...
                  <td class="text-right">
                    <button class="btn btn-ghost btn-sm" onclick="CostPage.openProductForm(${p.id})">수정</button>
                    <button class="btn btn-ghost btn-sm" onclick="CostPage.deleteProduct(${p.id})">삭제</button>
                  </td>
                </tr>
              `).join('')}
            </tbody>
  ...
  openProductForm(id = null) {
    ...
    Modal.open({
      ...
      footer: `
        ...
        <button class="btn btn-primary" onclick="CostPage.saveProduct(${id || 'null'})">저장</button>
      `
    });
  },
          ...
          <div class="card-body">
            <div class="form-grid">
              <div class="form-field full">
                ...
                <input class="form-control mono" id="calc-mw" type="number" value="30" oninput="CostPage.calculate()">
              </div>
              <div class="form-field">
                <label class="form-label">사업 유형</label>
                <select class="form-control" id="calc-type" onchange="CostPage.calculate()">
              ...
              <div class="form-field">
                ...
                <select class="form-control" id="calc-install" onchange="CostPage.calculate()">
  ...
  async renderHistory() {
    const result = await API.products.list();
    const tracked = result.data.filter(p => p.id <= 6).slice(0, 6);
