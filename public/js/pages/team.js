...
const TeamPage = {
  ...
  async render() {
    const html = `
      <div class="filter-bar">
        ...
        <button class="btn btn-primary" onclick="TeamPage.openForm()">+ 팀원 추가</button>
      </div>
        ...
        <tbody>
          ${this.members.map(m => `
            <tr>
              ...
              <td>
                <button class="btn btn-ghost btn-sm" onclick="TeamPage.openForm(${m.id})">편집</button>
              </td>
            </tr>
          `).join('')}
        </tbody>
  ...
  openForm(id = null) {
    ...
    Modal.open({
      ...
      `,
      footer: `
        ${m ? `<button class="btn btn-ghost text-danger" onclick="TeamPage.deactivate(${m.id})">비활성화</button>` : ''}
        <button class="btn btn-ghost" onclick="Modal.close()">취소</button>
        <button class="btn btn-primary" onclick="TeamPage.save(${m?.id || 'null'})">${m ? '저장' : '등록'}</button>
      `
    });
  },
