...
const ROLE_PAGES = {
  manager:    ['dashboard','pipeline','leads','customers','calendar','meeting','meeting-list','board','settings'],
  team_lead:  ['dashboard','pipeline','leads','customers','calendar','team','reports','meeting','meeting-list','board','settings','projects'],
  executive:  ['dashboard','pipeline','leads','customers','calendar','team','reports','meeting','meeting-list','board','settings','projects','admin'],
  admin:      ['dashboard','pipeline','leads','customers','calendar','team','reports','meeting','meeting-list','board','settings','projects','admin'],
  superadmin: ['*'],
};
...
async function loadBlacklistFromDB(pool) {
  ...
  } catch (_) { /* 테이블 미생성 시 무시 */ }
}

// ── OTP (secret은 AES-256-GCM으로 암호화하여 DB 저장) ────────
function generateOtpSecret()  { return otplib.generateSecret(); }
function encryptOtpSecret(s)  { return encrypt(s); }     // DB 저장 전 호출
function decryptOtpSecret(s)  { return decrypt(s); }     // DB에서 읽은 후 호출
...
// ── RBAC ─────────────────────────────────────────────────────
function getRoleInfo(role)           { return ROLES[role] || ROLES.manager; }
...
module.exports = {
  ...
  blacklistAdd, blacklistHas, loadBlacklistFromDB,
