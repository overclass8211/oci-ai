...
const spec = {
  ...
  components: {
    ...
    schemas: {
      ...
      Lead: {
        ...
        properties: {
          ...
          stage:            { type: 'string', enum: ['lead','review','proposal','bidding','negotiation','won','lost','dropped'] },
    ...
    parameters: {
      ...
      LimitParam: { name: 'limit', in: 'query', schema: { type: 'integer', default: 50, minimum: 1, maximum: 200 } },
    },
  },
  security: [{ bearerAuth: [] }],
  paths: {
    ...
    '/health': {
      get: {
        tags: ['Health'], summary: '헬스체크', security: [],
        responses: {
          200: { description: '정상', content: { 'application/json': { schema: { type: 'object', properties: {
    ...
    '/auth/login': {
      post: {
        ...
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['username','password'],
    ...
    // ── Dashboard ──────────────────────────────────────────
    '/dashboard/stats': {
      get: { tags: ['Dashboard'], summary: 'KPI 지표 조회',
        responses: { 200: { description: 'monthlyNew, domestic, overseas, totalLeads, bidding, wonAmount, winRate' } } },
    },
    '/dashboard/funnel': {
      get: { tags: ['Dashboard'], summary: '파이프라인 단계별 현황',
        responses: { 200: { description: 'stage별 count·amount 배열' } } },
    },
    '/dashboard/monthly': {
      get: { tags: ['Dashboard'], summary: '월별 영업기회 추이 (최근 6개월)',
        responses: { 200: { description: 'month·business_type·count 배열' } } },
    },
    '/dashboard/activities': {
      get: { tags: ['Dashboard'], summary: '최근 영업 활동 10건',
        responses: { 200: { description: '활동 배열' } } },
    },
    // ── Leads ──────────────────────────────────────────────
    '/leads': {
      get: {
        tags: ['Leads'], summary: '영업기회 목록 조회 (페이지네이션)',
        parameters: [
        ...
        responses: { 200: { description: '리드 목록 + pagination', content: { 'application/json': { schema: {
          allOf: [
      ...
      post: {
        tags: ['Leads'], summary: '영업기회 등록',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['customer_name','project_name'],
          properties: {
            customer_name:  { type: 'string' },
        ...
        responses: { 200: { description: '등록된 id 반환' }, 400: { description: '필수 항목 누락' } },
      },
    },
    '/leads/{id}': {
      get: {
        tags: ['Leads'], summary: '영업기회 상세 + 활동 이력',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        responses: { 200: { description: 'lead + activities 배열' }, 404: { description: '존재하지 않음' } },
      },
      put: {
        tags: ['Leads'], summary: '영업기회 수정',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/Lead' } } } },
        responses: { 200: { description: '수정 완료' } },
      },
      delete: {
        tags: ['Leads'], summary: '영업기회 삭제',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
    ...
    '/leads/{id}/stage': {
      patch: {
        tags: ['Leads'], summary: '영업기회 단계 변경',
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'integer' } }],
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['stage'],
          properties: { stage: { type: 'string', enum: ['lead','review','proposal','bidding','negotiation','won','lost','dropped'] } } } } } },
    ...
    '/customers': {
      get: {
        tags: ['Customers'], summary: '고객사 목록 (페이지네이션)',
        parameters: [
          { $ref: '#/components/parameters/PageParam' },
          { $ref: '#/components/parameters/LimitParam' },
          { name: 'search', in: 'query', schema: { type: 'string' } },
      ...
      post: {
        tags: ['Customers'], summary: '고객사 등록',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['name'],
          properties: { name: { type: 'string' }, region: { type: 'string' }, industry: { type: 'string' } } } } } },
    ...
    '/projects': {
      get: {
        tags: ['Projects'], summary: '프로젝트 목록 (페이지네이션)',
        parameters: [
          { $ref: '#/components/parameters/PageParam' },
          { $ref: '#/components/parameters/LimitParam' },
      ...
      post: { tags: ['Projects'], summary: '프로젝트 등록',
    ...
    '/products': {
      get: {
        tags: ['Products'], summary: '제품·원자재 목록 (페이지네이션)',
        parameters: [
          { $ref: '#/components/parameters/PageParam' },
          { $ref: '#/components/parameters/LimitParam' },
    ...
    '/team': {
      get: {
        tags: ['Team'], summary: '팀원 목록 + 영업 실적 (페이지네이션)',
        parameters: [
          { $ref: '#/components/parameters/PageParam' },
          { $ref: '#/components/parameters/LimitParam' },
        ],
    ...
    '/ai/chat': {
      post: {
        tags: ['AI'], summary: 'AI 어시스턴트 채팅 (SSE 스트리밍)',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object',
          properties: { messages: { type: 'array', items: { type: 'object' } } } } } } },
        responses: { 200: { description: 'text/event-stream — data: {"text":"..."}\\ndata: [DONE]' } },
      },
    },
    '/ai/report': {
      post: {
        tags: ['AI'], summary: '주간·월간 보고서 생성 (SSE)',
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object',
