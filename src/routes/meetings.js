  ...
  try {
    ...
    const [calMain] = await pool.query(
      ...
      [meetingTitle, md.substring(0, 500),
       ...
       customer_name || meeting.customer_name || null, '#1a73e8']);
    ...
    for (let i = 0; i < todoLines.length; i++) {
      ...
      await pool.query(
        ...
        [`[액션] ${customer_name || meeting.customer_name || ''} ${line}`.substring(0, 200),
         ...
         0, '기타', 'planned',
         lead_id || meeting.lead_id || null,
         customer_name || meeting.customer_name || null, '#fd7e14']);
      actionEventCount++;
    }
    ...
    res.json({ success: true, data: { main_event_id: calMain.insertId, action_events_created: actionEventCount } });
  } catch (err) { handleError(res, err); }
});
