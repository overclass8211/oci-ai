...
const {
  ...
  signRefreshToken, hashRefreshToken, verifyRefreshToken,
  blacklistAdd,
  generateOtpSecret, encryptOtpSecret, decryptOtpSecret, generateOtpUri, verifyOtp,
  getRoleInfo, ROLE_PAGES,
...
router.post('/login', schema(SCHEMAS.login), async (req, res) => {
  try {
    ...
    if (user.otp_enabled)
      return res.json({ success: true, requireOtp: true, userId: user.id });
...
router.post('/login-otp', async (req, res) => {
  try {
    ...
    if (!user.otp_secret) return res.status(400).json({ success: false, error: 'OTP 미설정 계정입니다.' });
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    for (const row of rows) {
      if (await verifyRefreshToken(raw, row.token_hash)) { matched = row; break; }
    }
    if (!matched)
    ...
    if (raw) {
      ...
      for (const row of rows) {
        if (await verifyRefreshToken(raw, row.token_hash)) {
          await pool.query('UPDATE refresh_tokens SET revoked=1, revoked_at=NOW() WHERE id=?', [row.id]);
          break;
        }
      }
    }
...
router.post('/logout-all', async (req, res) => {
  try {
    ...
    let decoded;
    try { decoded = verifyToken(token); } catch (_) {
...
router.post('/force-logout/:userId', async (req, res) => {
  try {
    ...
    let caller;
    try { caller = verifyToken(token); } catch (_) {
      return res.status(401).json({ success: false, error: '유효하지 않은 토큰' });
    }

    const { getRoleInfo } = require('../services/authService');
    if (getRoleInfo(caller.role).level < 4)
      return res.status(403).json({ success: false, error: '관리자(superadmin) 권한이 필요합니다.' });
...
router.get('/me', async (req, res) => {
  try {
    ...
    catch (_) { return res.status(401).json({ success: false, error: '세션 만료' }); }
    ...
    const roleInfo = getRoleInfo(user.role);
    const pages    = ROLE_PAGES[user.role] || ROLE_PAGES.manager;
    res.json({ success: true, data: { ...user, roleLabel: roleInfo.label, roleColor: roleInfo.color, pages } });
  } catch (err) { handleError(res, err); }
});
...
router.post('/enable-otp', async (req, res) => {
  try {
    const { userId, otpToken } = req.body;
    const [[user]] = await pool.query('SELECT * FROM users WHERE id=?', [userId]);
    if (!user || !user.otp_secret) return res.status(400).json({ success: false, error: 'OTP 설정 먼저 필요' });
