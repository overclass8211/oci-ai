...
router.post('/bulk', async (req, res) => {
  ...
  for (const row of projects) {
    if (!row.name) { errors.push({ row, reason: '프로젝트명 누락' }); continue; }
    try {
      const margin = (row.contract_amount && row.estimated_cost)
        ? (((row.contract_amount - row.estimated_cost) / row.contract_amount) * 100).toFixed(2) : null;
      const [r] = await pool.query(
        `INSERT INTO projects
         (name, customer_name, project_type, contract_amount, estimated_cost,
          margin_pct, status, due_date, assigned_to, notes)
         VALUES (?,?,?,?,?,?,?,?,?,?)`,
        [row.name, row.customer_name || null, row.project_type || '태양광',
         row.contract_amount || null, row.estimated_cost || null, margin,
         row.status || '진행중', row.due_date || null,
         row.assigned_to || null, row.notes || null]
      );
...
router.post('/import', upload.memory.single('file'), async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (!name) { errors.push({ row, reason: '프로젝트명 누락' }); continue; }
      try {
        ...
        const margin = (contractAmt && estimatedCost)
          ? (((contractAmt - estimatedCost) / contractAmt) * 100).toFixed(2) : null;
        ...
        const [r] = await pool.query(
          ...
          [name,
           ...
           contractAmt, estimatedCost, margin,
