...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    return res.status(401).json({ success: false, error: '세션이 만료되었습니다. 다시 로그인하세요.' });
  }
}
...
async function getAuthenticatedClient(userId) {
  ...
  if (!row || !row.refresh_token) throw Object.assign(new Error('Google 계정이 연결되지 않았습니다'), { notConnected: true });
  ...
  oauth2Client.on('tokens', async (tokens) => {
    ...
    if (tokens.access_token) { updates.push('access_token=?'); vals.push(encrypt(tokens.access_token)); }
    if (tokens.expiry_date)  { updates.push('expiry_date=?');  vals.push(tokens.expiry_date); }
    if (updates.length) {
      vals.push(userId);
      await pool.query(`UPDATE google_oauth_tokens SET ${updates.join(',')} WHERE user_id=?`, vals).catch(() => {});
    }
  });
  return oauth2Client;
}
...
function checkConfig(res) {
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    res.status(400).json({
      success: false,
      error: 'Google OAuth가 설정되지 않았습니다. .env 파일에 GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET을 입력하세요.',
      notConfigured: true
    });
    return false;
  }
...
router.get('/status', requireAuth, async (req, res) => {
  try {
    ...
    const [[row]] = await pool.query(
      'SELECT google_email, updated_at FROM google_oauth_tokens WHERE user_id = ?', [userId]
    );
    res.json({ success: true, data: { connected: !!row, configured: true, email: row?.google_email || null } });
...
function popupHtml({ success, email, error }) {
  const payload = JSON.stringify({ type: 'google_oauth', success, email: email || null, error: error || null });
...
router.post('/meet/create', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!meetLink) return res.status(500).json({ success: false, error: 'Meet 링크 생성 실패 (Google Workspace 계정 필요)' });
    ...
    res.json({
      success: true,
      data: { id: result.insertId, meet_link: meetLink, google_event_id: googleEventId,
              title: title || '영업 미팅', scheduled_at: startDt, duration_min }
    });
  } catch (err) {
    if (err.notConnected) return res.status(401).json({ success: false, error: err.message, notConnected: true });
