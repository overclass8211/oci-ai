  ...
  try {
    ...
    const [[totalLeads]] = await pool.query(
      `SELECT COUNT(*) AS count FROM leads
       WHERE stage NOT IN ('won','lost','dropped') AND YEAR(created_at) = ?`, [year]);

    // 이번달 or 선택연도 마지막달 신규
    const [[monthlyNew]] = isCurrentYear
      ? await pool.query(
          `SELECT COUNT(*) AS count FROM leads
           WHERE YEAR(created_at)=? AND MONTH(created_at)=MONTH(CURRENT_DATE())`, [year])
      : await pool.query(
          `SELECT COUNT(*) AS count FROM leads
           WHERE YEAR(created_at)=? AND MONTH(created_at)=12`, [year]);
    ...
    const [[wonAmount]] = await pool.query(
      `SELECT COALESCE(SUM(COALESCE(amount_krw, IF(currency='KRW', expected_amount, 0))),0) AS amount
       FROM leads
       WHERE stage = 'won' AND YEAR(created_at) = ?`, [year]);

    // 입찰 진행 — 선택연도
    const [[bidding]] = await pool.query(
      `SELECT COUNT(*) AS count FROM leads WHERE stage='bidding' AND YEAR(created_at)=?`, [year]);

    // 국내/해외 (파이프라인)
    const [[domestic]] = await pool.query(
      `SELECT COUNT(*) AS count FROM leads
       WHERE region='국내' AND stage NOT IN ('won','lost','dropped') AND YEAR(created_at)=?`, [year]);
    const [[overseas]] = await pool.query(
      `SELECT COUNT(*) AS count FROM leads
       WHERE region='해외' AND stage NOT IN ('won','lost','dropped') AND YEAR(created_at)=?`, [year]);

    // 수주율 = 해당연도 생성 리드 중 수주 / 해당연도 전체 (created_at 기준 통일)
    const [[wonCount]] = await pool.query(
      `SELECT COUNT(*) AS count FROM leads WHERE stage='won' AND YEAR(created_at)=?`, [year]);
    const [[allCount]] = await pool.query(
      `SELECT COUNT(*) AS count FROM leads WHERE YEAR(created_at)=?`, [year]);
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT stage, COUNT(*) AS count, COALESCE(SUM(expected_amount),0) AS amount
       FROM leads WHERE YEAR(created_at) = ? GROUP BY stage`, [year]);
    ...
    if (period === 'annual') {
      ...
      [rows] = await pool.query(
        `SELECT CONCAT('Q', QUARTER(created_at)) AS qtr, business_type, COUNT(*) AS count
         ...
         GROUP BY qtr, business_type ORDER BY qtr`, [year]);
    } else if (period === 'monthly') {
      // 선택 연도의 12개월 전체
      [rows] = await pool.query(
        `SELECT DATE_FORMAT(created_at, '%Y-%m') AS month, business_type, COUNT(*) AS count
         FROM leads WHERE YEAR(created_at) = ?
         GROUP BY month, business_type ORDER BY month`, [year]);
    } else {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT a.*, t.name AS performer_name, l.customer_name, l.project_name
       ...
       ORDER BY a.performed_at DESC LIMIT 10`, [year]);
    res.json({ success: true, data: rows });
  } catch (err) { handleError(res, err); }
});
