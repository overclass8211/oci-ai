...
const { requireFields, validateId, schema, SCHEMAS } = require('../middleware/validate');
...
const {
  genAI, MODEL_FAST, SAFETY_SETTINGS, logTokenUsage, runStream, sseStart, sseError
...
router.post('/bulk', async (req, res) => {
  ...
  const inserted = []; const errors = []; const duplicates = [];
  for (const row of customers) {
    if (!row.name) { errors.push({ row, reason: '고객사명 누락' }); continue; }
    try {
      const dup = await findDuplicate(row.name, row.contact_person, row.phone);
      if (dup) {
        duplicates.push({ row, existingId: dup.id,
      ...
      const [r] = await pool.query(
        `INSERT INTO customers (name, region, country, industry, contact_person, phone, email, address)
         VALUES (?,?,?,?,?,?,?,?)`,
        [row.name, row.region || '국내', row.country || null, row.industry || null,
         row.contact_person || null, row.phone || null, row.email || null, row.address || null]
      );
...
router.post('/', schema({ name: { type:'string', required:true, minLen:1, maxLen:200 }, region: { type:'string', maxLen:100 } }), async (req, res) => {
  try {
  ...
  for (const file of req.files) {
    try {
      ...
      try { parsed = JSON.parse(text); } catch (_) {
        ...
        if (m) { try { parsed = JSON.parse(m[0]); } catch (__) { /* fallback parse failed, use empty object */ } }
      }
  ...
  try {
    ...
    const [deals] = await pool.query(
      `SELECT id, project_name, business_type, region, stage,
       ...
       FROM leads WHERE customer_name = ? ORDER BY updated_at DESC`, [c.name]);
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, name, region, country, industry, contact_person, phone, email
       FROM customers WHERE name=? ORDER BY id`, [c.name]);
  ...
  try {
    ...
    const [acts] = await pool.query(
      `SELECT a.activity_type, a.title, a.performed_at, t.name AS performer
       FROM activities a JOIN leads l ON a.lead_id=l.id
       LEFT JOIN team_members t ON a.performed_by=t.id
       WHERE l.customer_name=? ORDER BY a.performed_at DESC LIMIT 5`, [customer.name]);
...
[최근 딜] ${deals.slice(0,3).map(d => `${d.project_name}(${stageMap[d.stage]||d.stage})`).join(', ')||'없음'}
[최근 활동] ${acts.slice(0,3).map(a => `${a.activity_type}: ${a.title}`).join(' / ')||'없음'}
...
다음 JSON 형식으로만 응답하세요 (마크다운/설명 없이 JSON만):
{
  ...
  "risk": "주의해야 할 리스크 한 줄 (없으면 null)"
    ...
    catch { return res.status(502).json({ success: false, error: 'AI 응답 파싱 실패', raw: txt.slice(0, 300) }); }
    ...
    try {
      ...
      const [[meta]] = await pool.query(
        `SELECT generated_at FROM customer_briefs WHERE id=?`, [r.insertId]);
...
router.put('/:id', validateId, async (req, res) => {
  try {
    const { name, region, country, industry, contact_person, phone, email, address, notes } = req.body;
    const fields = [];
    const vals   = [];
    ...
    if (industry       !== undefined) { fields.push('industry=?');       vals.push(industry); }
    ...
    if (!fields.length) return res.status(400).json({ success: false, error: '수정할 항목이 없습니다.' });
...
router.get('/export', async (req, res) => {
  try {
    ...
    const buf = toExcelBuffer(CUST_COLS, rows, '고객사');
    sendExcel(res, buf, '고객사_' + new Date().toISOString().slice(0, 10));
  } catch (err) { handleError(res, err); }
});
...
router.post('/import', upload.memory.single('file'), async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (!name) { errors.push({ row, reason: '고객사명 누락' }); continue; }
      try {
