...
const {
  ...
  PageBreak, ShadingType, LevelFormat, Footer, Header, PageNumber,
} = require('docx');
...
const TABLE_KO = {
  ...
  pipeline_stages:'파이프라인 단계', exchange_rates:'환율 시계열',
  customer_briefs:'고객 AI 브리핑', schema_change_log:'스키마 변경 이력',
  _migrations:'마이그레이션 마커',
};

// ── 색상 헬퍼 ─────────────────────────────────────────────
const COLOR = {
  primary: '1664E5', accent: '7C4DFF', text: '111827',
  textLight: '6B7280', bgLight: 'F3F4F6', border: 'E5E7EB',
  red: 'E63329', green: '17A85A', orange: 'F59C00',
};
...
function buildCoverPage() {
  ...
  return [
    ...
    new Paragraph({
      ...
      children: [new TextRun({ text:'🔥 핑거세일즈 AI', bold:true, size:72, color:COLOR.primary, font:'Malgun Gothic' })],
    }),
    new Paragraph({ children: [new TextRun({ text:'', size: 24 })] }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text:'DB 테이블 정의서', bold:true, size:60, color:COLOR.text, font:'Malgun Gothic' })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text:'Database Schema Definition Document', size:28, color:COLOR.textLight, italics:true, font:'Malgun Gothic' })],
    }),
    new Paragraph({ children: [new TextRun({ text:'', size: 24 })] }),
    new Paragraph({ children: [new TextRun({ text:'', size: 24 })] }),
    new Paragraph({ children: [new TextRun({ text:'', size: 24 })] }),
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text:`발행일: ${ymd}`, size:24, color:COLOR.textLight, font:'Malgun Gothic' })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text:'Confidential · For Internal Use Only', size:20, color:COLOR.textLight, font:'Malgun Gothic' })],
    }),
...
function buildToc(tables) {
  ...
  return [
    new Paragraph({
      ...
      children: [new TextRun({ text:'📋 목차', bold:true, size:36, color:COLOR.primary, font:'Malgun Gothic' })],
    }),
    new Paragraph({
      children: [new TextRun({ text:`총 ${tables.length}개 테이블`, size:22, color:COLOR.textLight, font:'Malgun Gothic' })],
    }),
...
function buildTablePage(table, fks) {
  ...
  parts.push(new Paragraph({
    heading: HeadingLevel.HEADING_1,
    children: [
      new TextRun({ text:`📂 ${table.name}`, bold:true, size:32, color:COLOR.primary, font:'Malgun Gothic' }),
      new TextRun({ text:`   ${TABLE_KO[table.name] || ''}`, size:24, color:COLOR.textLight, font:'Malgun Gothic' }),
    ],
  }));
  ...
  parts.push(new Paragraph({
    ...
    children: [
      new TextRun({ text:`컬럼 ${table.colCount}개  ·  데이터 ${table.rowCount || 0}건  ·  FK 출 ${fksOut.length} / FK 입 ${fksIn.length}`, size:20, color:COLOR.textLight, font:'Malgun Gothic' }),
    ],
  }));
  ...
  table.columns.forEach((c, i) => {
    const keyDesc = c.COLUMN_KEY === 'PRI' ? 'PK'
                  : c.COLUMN_KEY === 'UNI' ? 'UNIQUE'
                  : c.COLUMN_KEY === 'MUL' ? 'INDEX' : '—';
    const keyColor = c.COLUMN_KEY === 'PRI' ? COLOR.primary
                   : c.COLUMN_KEY === 'UNI' ? COLOR.accent : COLOR.textLight;
    colRows.push(new TableRow({
      children: [
        ...
        cell(c.IS_NULLABLE === 'YES' ? 'YES' : 'NO',
             { center:true, size:14, color: c.IS_NULLABLE === 'NO' ? COLOR.red : COLOR.orange }),
        cell(c.COLUMN_DEFAULT === null || c.COLUMN_DEFAULT === undefined ? '—' : String(c.COLUMN_DEFAULT),
  ...
  parts.push(new Paragraph({
    children: [new TextRun({ text:'📊 컬럼 정의', bold:true, size:24, color:COLOR.text, font:'Malgun Gothic' })],
  ...
  if (fksOut.length > 0) {
    parts.push(new Paragraph({
      children: [new TextRun({ text:'🔗 FK 출 (이 테이블이 참조하는 외부 테이블)', bold:true, size:22, color:COLOR.text, font:'Malgun Gothic' })],
  ...
  if (fksIn.length > 0) {
    parts.push(new Paragraph({
      children: [new TextRun({ text:'🔗 FK 입 (이 테이블을 참조하는 외부 테이블)', bold:true, size:22, color:COLOR.text, font:'Malgun Gothic' })],
      spacing: { before: 300, after: 100 },
    }));
    const fkInRows = [
      new TableRow({
  ...
  try {
    ...
    const [allFks] = await pool.query(
      `SELECT k.TABLE_NAME, k.COLUMN_NAME, k.REFERENCED_TABLE_NAME, k.REFERENCED_COLUMN_NAME,
       ...
       WHERE k.TABLE_SCHEMA=? AND k.REFERENCED_TABLE_NAME IS NOT NULL`, [dbName]);
        ...
        headers: {
          default: new Header({
            children: [new Paragraph({
              ...
              children: [new TextRun({ text:'핑거세일즈 AI · DB 테이블 정의서', size:14, color:COLOR.textLight, font:'Malgun Gothic' })],
            })],
          }),
        },
        footers: {
          default: new Footer({
            children: [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [
                new TextRun({ text:'- ', size:14, color:COLOR.textLight, font:'Malgun Gothic' }),
                new TextRun({ children:[PageNumber.CURRENT], size:14, color:COLOR.textLight }),
                new TextRun({ text:' / ', size:14, color:COLOR.textLight, font:'Malgun Gothic' }),
                new TextRun({ children:[PageNumber.TOTAL_PAGES], size:14, color:COLOR.textLight }),
                new TextRun({ text:' -', size:14, color:COLOR.textLight, font:'Malgun Gothic' }),
              ],
            })],
          }),
