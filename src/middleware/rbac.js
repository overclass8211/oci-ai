'use strict';
const { verifyToken, getRoleInfo, getRequiredLevel, blacklistHas } = require('../services/authService');
...
async function authenticate(req, res, next) {
  ...
  try {
    ...
    const msg = err.name === 'TokenExpiredError'
      ...
      : '유효하지 않은 인증 토큰입니다.';
    return res.status(401).json({ success: false, error: msg, expired: err.name === 'TokenExpiredError' });
  }
  ...
  if (decoded.jti) {
    ...
    if (blacklistHas(decoded.jti)) {
      return res.status(401).json({ success: false, error: '만료된 세션입니다. 다시 로그인하세요.', revoked: true });
    }
    // DB 확인 (서버 재시작 후 메모리 캐시 없는 경우 대비)
    try {
      const [[bl]] = await pool.query(
        'SELECT jti FROM token_blacklist WHERE jti = ? AND expires_at > NOW()', [decoded.jti]
      );
      if (bl) {
